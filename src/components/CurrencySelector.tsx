...
export function CurrencySelector(props: CurrencySelectorProps) {
  ...
  return <div>
    <Chip
      onClick={handleClick}
      icon={<Icon name={props.value} size={25} />}
    ...
    >
      {props.options.map(choice => <MenuItem key={choice} onClick={() => {
        props.onChange(choice)
        handleClose()
      }}>
        <ListItemIcon><Icon name={choice} size={25} /></ListItemIcon> {choice.toUpperCase()}
      </MenuItem>)}
    </Menu>
  </div>;
}
